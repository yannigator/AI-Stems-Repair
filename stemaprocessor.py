...
def apply_dynamic_de_esser(
    ...
    """Applies dynamic bandpass de-essing to attenuate sibilance."""
    if np.all(audio == 0) or mix <= 0.0:
        return audio
    ...
    def process_channel(ch: np.ndarray) -> np.ndarray:
        ...
        if ratio > 1.0:
            gain_db[over_thresh] = -(env_db[over_thresh] - threshold_db) * (1.0 - (1.0 / ratio))
...
def apply_vocal_body_enhancer(
    ...
    """Applies dynamic harmonic saturation to body frequencies."""
    if mix <= 0.0 or np.all(audio == 0):
        return audio

    drive = 10 ** (drive_db / 20.0)
    sos_warmth = butter(2, [freq_low, freq_high], btype="bandpass", fs=sr, output="sos")
...
def apply_ms_stereo_widener(
    ...
    """Applies Mid/Side matrix gain adjustments to control spatial width."""
...
def apply_gentle_volume_ride(
    ...
    """Smoothly normalizes long-term dynamic fluctuations without squishing transients."""
    if np.all(audio == 0):
        return audio
...
# =====================================================================
# 2. MAIN STEM PROCESSING PIPELINE
# =====================================================================
def repair_ai_vocal(
    input_path: str,
    output_path: str,
    ...
    """Executes full cleaning, saturation, stereo, and level correction pipeline on a single file."""
    ...
    audio = apply_dynamic_de_esser(
        audio, sr, threshold_db=deesser_threshold, ratio=deesser_ratio, mix=deesser_mix
    )

    # 3. Vocal Body / Warmth Enhancer
    audio = apply_vocal_body_enhancer(
        audio, sr, drive_db=warmth_drive, mix=warmth_mix, adaptive=True
    )

    # 4. M/S Stereo Widener
    if audio.shape[0] >= 2:
    ...
    # 6. Global Peak Normalization (-1.0 dBFS headroom)
    max_peak = np.max(np.abs(audio))
    if max_peak > 0:
        target_linear = 10 ** (-1.0 / 20.0)
        audio = audio * (target_linear / max_peak)

    # Metadata & Dynamic Range Analysis
...
# =====================================================================
# 3. STEM PRESETS & COMBINING HELPERS
# =====================================================================
STEM_PRESETS = {
    "backing vocal": {
...
def combine_stems(processed_files: list, output_combined_path: str):
    """Sums multiple stem files into a single master WAV file normalized to -1.0 dBFS."""
